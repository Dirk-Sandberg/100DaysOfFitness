...
class FirebaseLoginScreen(Screen, EventDispatcher):
    ...
    def on_web_api_key(self, *args):
        ...
        if os.path.exists(self.refresh_token_file):
            ...
            self.ids.welcome_screen.ids.log_in_button.opacity = 0
            self.ids.welcome_screen.ids.create_account_button.opacity = 0
    ...
    def sign_up(self, email, password):
        ...
        UrlRequest(signup_url, req_body=signup_payload,
                   on_success=self.successful_signup,
                   on_failure=self.sign_up_failure,
                   on_error=self.sign_up_error, ca_file=certifi.where())

    def successful_signup(self, urlrequest, log_in_data):
        ...
        UrlRequest(App.get_running_app().firebase_url + "num_users.json",
                   on_success=self.add_to_user_count, ca_file=certifi.where())

    def add_to_user_count(self, req, *args):
        try:
            print("Trying to new count", req.result)
            new_count = req.result + 1
        except:
            new_count = 1
        # Add to the total user count
        new_count = '{"num_users": %s}' %new_count
        UrlRequest(App.get_running_app().firebase_url + ".json",
    ...
    def failed_account_load(self, *args):
        ...
        self.ids.welcome_screen.ids.log_in_button.opacity = 1
        self.ids.welcome_screen.ids.create_account_button.opacity = 1
