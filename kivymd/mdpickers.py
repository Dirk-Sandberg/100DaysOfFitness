...
<StandardMDDatePicker>
    auto_dismiss: False#True if app.is_editing_start_date else False
    cal_layout: cal_layout
    size_hint: (None, None)
    background_color: app.theme_cls.primary_color#Window.clearcolor
    size: 
    ...
    background_normal: ""
    _anim_duration: .2
    #_anim_transition: 'in_out_quad'
    opacity: self._anim_alpha# if self.is_opening else self._anim_alpha**4#self.opacity
    #size: dp(328), dp(120)+dp(290)
    ...
    MDLabel:
        ...
        valign: 'middle'
        text: "Choose your start date"#str(root.sel_year)
    ...
    MDFlatButton:
        ...
        opacity: 1 if app.is_editing_start_date else 0
        disabled: False if app.is_editing_start_date else True
...
class StandardDayButton(ThemableBehavior, CircularRippleBehavior, ButtonBehavior,
                AnchorLayout):
    text = StringProperty()
...
class StandardMDDatePicker(FloatLayout, ThemableBehavior, RectangularElevationBehavior,
    ...
    is_opening = BooleanProperty(False)
    ...
    def generate_cal_widgets(self):
        ...
        for i in range(6 * 7):  # 6 weeks, 7 days a week
            db = StandardDayButton(owner=self)
            cal_list.append(db)
...
class StandardMDThemePicker(ThemableBehavior, FloatLayout, ModalView,
